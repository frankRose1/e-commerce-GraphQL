...
describe("<Order />", () => {
  ...
  it("properly displays order information", async () => {
    ...
    expect(wrapper.find('[data-test="order-total"]').text()).toEqual(formatMoney(order.total));
    expect(wrapper.find('[data-test="order-charge"]').text()).toEqual(order.charge);
  });
  ...
  it("shows the correct amount of items in the order", async () => {
    ...
    await wait();
    wrapper.update();
    expect(wrapper.find('div.items').children()).toHaveLength(order.items.length);
  });
});