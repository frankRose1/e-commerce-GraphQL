...
const {requiresLogin, hasPermission} = require('../utils');
...
const Mutations = {
  ...
  async updateItem(parent, args, ctx, info){
    ...
    const updates = { ...args };
    // remove the ID because it is also coming in on the request
    delete updates.id;
  ...
  async deleteItem(parent, args, ctx, info){
    ...
    //find item and ask for the owner of the item
    ...
    // see if user owns it or has permission
    const ownsItem = item.user.id = ctx.request.userId;
    const permissionGranted = ctx.request.user.permissions.some(permission => ['ADMIN', 'ITEMDELETE'].includes(permission));
    if (!ownsItem || !permissionGranted) {
      throw new Error('Hey, you\'re not allowed to do that!');
    }
    //delete the item
  ...
  async requestReset(parent, {email}, ctx, info){
    ...
    return {message: 'You have been sent an email with instructions on resetting your password.'}
  },
